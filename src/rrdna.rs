...
pub fn rrdna_search(id: &str) -> Result<String, Box<dyn Error>> {
        ...
        for data in datasets.1.iter() {
            for val in genuspathunwrap.1.iter() {
                for species in speciespathunwrap.1.iter() {
                    if data.rrdna16s_accession.contains(id)
                        || val.rrdna16s_accession.contains(id)
                        || species.rrdna16s_accession.contains(id)
                    {
