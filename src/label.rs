...
pub fn label_search(id: &str) -> Result<String, Box<dyn Error>> {
        ...
        for data in datasets.1.iter() {
            for val in genuspathunwrap.1.iter() {
                for species in speciespathunwrap.1.iter() {
                    if data.ncbi_associated_level.contains(id)
                        || val.ncbi_associated_level.contains(id)
                        || species.ncbi_associated_level.contains(id)
                    {
