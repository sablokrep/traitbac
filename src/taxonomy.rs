...
pub fn taxonomy(id: &str) -> Result<String, Box<dyn Error>> {
        ...
        for data in datasets.1.iter() {
            for val in genuspathunwrap.1.iter() {
                for species in speciespathunwrap.1.iter() {
                    if data.taxonomy_upd_method.contains(id)
                        || val.taxonomy_upd_method.contains(id)
                        || species.taxonomy_upd_method.contains(id)
                    {
